...
function igstalk(username) {
    return new Promise((resolve, reject) => {
        const url = `https://www.instagram.com/${username}/?__a=1`; // Menggunakan URL yang valid dengan parameter `__a=1`
        
        axios.get(url)
        .then(({ data }) => {
            // Mengambil data dari respons API Instagram
            ...
            resolve(result); // Mengembalikan hasil dalam bentuk JSON
        })
        .catch(reject); // Menangani error jika permintaan gagal
    });
}
